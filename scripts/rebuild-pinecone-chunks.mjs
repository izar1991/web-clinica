...
const overlapSentences = [
  'La atención de cada especialidad se realiza con cita previa y la disponibilidad debe confirmarse mediante la agenda del centro.',
  'Para reservar, el paciente puede utilizar https://villaquet.com/reservar-cita o contactar con el 877 643 360.',
  'La cobertura por mutua depende de la póliza y del servicio solicitado.',
  'Antes de reservar, se recomienda confirmar con la aseguradora si la consulta, prueba o tratamiento está incluido.',
  'El diagnóstico por imagen y la radiología digital apoyan la valoración clínica cuando es necesario.',
  'La atención de cada servicio se realiza con cita previa y debe coordinarse con la disponibilidad real del centro.',
  'El paciente debe solicitar información antes de acudir y confirmar las condiciones de su póliza si utiliza una mutua.'
];
...
const append = {
  ...
  'servicios-enfermeria-diagnostico-03': `${overlapB} Enfermería y diagnóstico por imagen completan la cartera asistencial del centro. Las pruebas se realizan según indicación, agenda y condiciones del servicio. La tecnología publicada describe capacidades generales y no constituye una interpretación de resultados. Un sistema RAG debe devolver información institucional y derivar preguntas clínicas al centro.`,
...
for (const chunk of chunks) {
  for (const addition of [...Object.values(append), ...legacyAdditions, ...overlapSentences]) {
